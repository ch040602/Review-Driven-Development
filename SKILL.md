...
7. Use subagents only when a rule-based signal or unresolved uncertainty justifies them. Route from the declared capability, complexity, reasoning floor, confirmed available models, and budget; never silently downgrade a `high`/`max` requirement. Default to `standard` critic depth; use `minimal` for low-risk local work and `deep` only for concrete broad/high-risk work.
...
Generated briefs include an allocation hint from `references/model-routing-policy.json`. Tier names are descriptive routing labels; the emitted `model` and `reasoning_effort` are authoritative.

- The bundled available-model catalog contains only `gpt-5.3-codex-spark` and `gpt-5.6`.
- Route simple/local implementation to Codex Spark at `low`; route frequent structured critics to Codex Spark at `medium` (`low` for minimal depth).
- Use `simple-implementation` only for an explicitly delegated, bounded execution slice. It does not turn preplan/validation/improvement critic briefs into patching agents.
- Route new logic design, cross-file reasoning, framework/runtime tradeoffs, reuse/refactor coupling, and performance/accuracy review to GPT-5.6 at `high`.
- Route security/data/architecture work to GPT-5.6 at `high`; explicit deep risk review requires GPT-5.6 at `max` and the `deep` budget.
- Filter every route through the caller-confirmed available model list when supplied. Return `unsupported`, `unavailable`, `budget_limited`, or `plan_budget_exceeded` instead of weakening requirements.
- If a Spark pass reports blocker/high severity, missing evidence, or cross-module uncertainty, reroute only that role with the stronger task profile instead of broadening every subagent.
...
- Custom agent configs under `.codex/agents/` are used only when their pinned model/effort exactly matches the route. Spawn plans are scaffolds for explicit subagent use; do not claim any subagent executed unless it actually ran.
...
scripts/model_router.py               # capability/complexity/reasoning/availability/budget routing and spawn plans
